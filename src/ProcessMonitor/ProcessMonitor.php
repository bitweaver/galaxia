...
class ProcessMonitor extends Base {
  ...
  function monitor_stats() {
    $res = Array();
    $res['active_processes'] = $this->getOne("select count(*) from `".GALAXIA_TABLE_PREFIX."processes` where `is_active`=?",array('y'));
    $res['processes'] = $this->getOne("select count(*) from `".GALAXIA_TABLE_PREFIX."processes`");
    $result = $this->query("select distinct(`p_id`) from `".GALAXIA_TABLE_PREFIX."instances` where `status`=?",array('active'));
  ...
  function monitor_list_activities($offset,$maxRecords,$sort_mode,$find,$where='') {
    ...
    while($res = $result->fetchRow()) {
      // Number of active instances
      $aid = $res['activity_id'];
      $res['active_instances']=$this->getOne("select count(gi.`instance_id`) from `".GALAXIA_TABLE_PREFIX."instances` gi, `".GALAXIA_TABLE_PREFIX."instance_activities` gia where gi.`instance_id`=gia.`instance_id` and gia.`activity_id`=$aid and gi.`status`='active' and `p_id`=".$res['p_id']);
    // activities of completed instances are all removed from the instance_activities table for some reason, so we need to look at workitems
      $res['completed_instances']=$this->getOne("select count(distinct gi.`instance_id`) from `".GALAXIA_TABLE_PREFIX."instances` gi, `".GALAXIA_TABLE_PREFIX."workitems` gw where gi.`instance_id`=gw.`instance_id` and gw.`activity_id`=$aid and gi.`status`='completed' and `p_id`=".$res['p_id']);
    // activities of aborted instances are all removed from the instance_activities table for some reason, so we need to look at workitems
      $res['aborted_instances']=$this->getOne("select count(distinct gi.`instance_id`) from `".GALAXIA_TABLE_PREFIX."instances` gi, `".GALAXIA_TABLE_PREFIX."workitems` gw where gi.`instance_id`=gw.`instance_id` and gw.`activity_id`=$aid and gi.`status`='aborted' and `p_id`=".$res['p_id']);
      $res['exception_instances']=$this->getOne("select count(gi.`instance_id`) from `".GALAXIA_TABLE_PREFIX."instances` gi, `".GALAXIA_TABLE_PREFIX."instance_activities` gia where gi.`instance_id`=gia.`instance_id` and gia.`activity_id`=$aid and gi.`status`='exception' and `p_id`=".$res['p_id']);
    $res['act_running_instances']=$this->getOne("select count(gi.`instance_id`) from `".GALAXIA_TABLE_PREFIX."instances` gi, `".GALAXIA_TABLE_PREFIX."instance_activities` gia where gi.`instance_id`=gia.`instance_id` and gia.`activity_id`=$aid and gia.`status`='running' and `p_id`=".$res['p_id']);      
    ...
    //  $res['act_completed_instances']=$this->getOne("select count(gi.instance_id) from ".GALAXIA_TABLE_PREFIX."instances gi,".GALAXIA_TABLE_PREFIX."instance_activities gia where gi.instance_id=gia.instance_id and gia.activity_id=$aid and gia.status='completed' and p_id=".$res['p_id']);      
  ...
  function monitor_list_instances($offset,$maxRecords,$sort_mode,$find,$where='',$wherevars=array()) {
    ...
    while($res = $result->fetchRow()) {
      $iid = $res['instance_id'];
      $res['workitems']=$this->getOne("select count(*) from `".GALAXIA_TABLE_PREFIX."workitems` where `instance_id`=?",array($iid));
      $ret[$iid] = $res;
    }
    $retval = Array();
    $retval["data"] = $ret;
  ...
  function monitor_list_all_processes($sort_mode = 'name_asc', $where = '') {
    ...
    $query = "select `name`,`version`,`p_id` from `".GALAXIA_TABLE_PREFIX."processes` $where order by ".$this->convert_sortmode($sort_mode);
    $result = $this->query($query);
    $ret = Array();
    while($res = $result->fetchRow()) {
      $p_id = $res['p_id'];
  ...
  function monitor_list_all_activities($sort_mode = 'name_asc', $where = '') {
    ...
    $query = "select `name`,`activity_id` from `".GALAXIA_TABLE_PREFIX."activities` $where order by ".$this->convert_sortmode($sort_mode);
    $result = $this->query($query);
    $ret = Array();
    while($res = $result->fetchRow()) {
      $aid = $res['activity_id'];
  ...
  function monitor_get_workitem($item_id) {
    ...
    $result = $this->query($query, array($item_id));
    $res = $result->fetchRow();
